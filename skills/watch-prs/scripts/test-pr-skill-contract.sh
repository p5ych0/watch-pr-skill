...
hasf 'watch-pr-skill:cold-reviewer' && hasf 'It is a pre-read, not a gate: nothing refuses when it is skipped' \
    && pass "…by the name the plugin ships it under, and as a pre-read that gates nothing" \
    || die "SKILL.md names the cold reviewer by no name the plugin loads, or makes it a gate"
