...
build_prompt() {
...

    {
        ...
        printf '%s\n' 'Put a NON-BLOCKING observation in summary, not in findings[]. Every findings[] entry becomes a review thread the merge gate requires resolved, so a note filed there blocks the merge however it is labelled. summary is RECORDED on EVERY review, including one that reports findings: it is kept verbatim in the bus response, so a concern you decline to force into a line-attached finding survives in the review record rather than being discarded.'
