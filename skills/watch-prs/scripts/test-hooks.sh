...
# The scratch projects above are not repositories, so status finds nothing to differ.
git init -q "$tmp/repo" && mkdir -p "$tmp/repo/skills/watch-prs/scripts"
printf '#!/usr/bin/env bash\nexit 0\n' > "$tmp/repo/skills/watch-prs/scripts/pr-selfcheck.sh"
chmod +x "$tmp/repo/skills/watch-prs/scripts/pr-selfcheck.sh"
git -C "$tmp/repo" -c user.email=t@example.invalid -c user.name=t add -A >/dev/null 2>&1
git -C "$tmp/repo" -c user.email=t@example.invalid -c user.name=t commit -qm one >/dev/null 2>&1
expect "$tmp/repo" "$(cmd 'git push origin b')" 0 "a push passes when the tree matches the commit"
printf '#!/usr/bin/env bash\nexit 0\n# edited\n' > "$tmp/repo/skills/watch-prs/scripts/pr-selfcheck.sh"
expect "$tmp/repo" "$(cmd 'git push origin b')" 2 "…and is blocked while a tracked file differs from it"
grep -q 'differ from the commit' "$tmp/err" && pass "…for that reason" || die "the difference was not named: $(head -c 160 "$tmp/err")"
printf 'untracked\n' > "$tmp/repo/new.txt"
git -C "$tmp/repo" -c user.email=t@example.invalid -c user.name=t checkout -- skills >/dev/null 2>&1
expect "$tmp/repo" "$(cmd 'git push origin b')" 0 "…while an untracked file, which no push sends, is not a difference"
