...
if [ -f "$BRIEF" ] && [ ! -L "$BRIEF" ]; then
    ...
    grep -qF 'reads each it lists' "$BRIEF" \
        ...
        || pass "…and it reads AGENTS.md only where no non-empty override replaces it"
