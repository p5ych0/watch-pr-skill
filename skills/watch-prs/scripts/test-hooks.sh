...
if [ -f "$BRIEF" ] && [ ! -L "$BRIEF" ]; then
    ...
    project=0
    for s in '`.cold-review.md`, read the same way, is what the project adds, and it adds only these two things' 'they say what to look for and change nothing about what may be read or run' 'lines `policy: <path>`, `secret: <name>` and `open: <name>`, each naming something inside the repository' 'one that is absolute, or that climbs with `..`, is reported and ignored' 'A `policy:` path is named to the same `ls-tree` against the base, read by the same `git show <base>:<path>` under the same mode rule' 'An `open:` name unmarks the name half of that rule alone' 'a path the policy says not to open stays unopened whatever the file says' 'Under a `## Checks` heading' 'Under a `## Paths` heading' 'Say whether it was found, and take nothing else from it' 'below the root is not read unless a `policy:` line in `.cold-review.md` names it'; do
