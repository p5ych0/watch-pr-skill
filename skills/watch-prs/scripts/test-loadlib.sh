...
got="$(run_case 'widget_fn() { :; }' '' widgetlib widget_fn "PR_X status=error")"
[ "$got" = LOADED ] && pass "a library defining its function loads" \
    || die "a well-formed function library did not load ('$got')"
got="$(run_case 'WIDGET_VAR="something"' '' widgetlib WIDGET_VAR "PR_X status=error" var)"
...
for kind_case in 'func:widget_fn:' 'var:WIDGET_VAR:var'; do
    ...
    sym="${rest%%:*}"; arg="${rest##*:}"
    got="$(run_case '' '' widgetlib "$sym" "PR_X status=error" ${arg:+"$arg"} 2>&1)"
...
got="$(run_case '' 'widget_fn() { printf stale; }; export -f widget_fn' widgetlib widget_fn "PR_X status=error")"
grep -q 'widgetlib_empty' <<<"$got" \
    && pass "an inherited FUNCTION does not satisfy an empty library" \
    || die "an exported function satisfied the load check ('$got')"
got="$(run_case '' 'export WIDGET_VAR=stale' widgetlib WIDGET_VAR "PR_X status=error" var)"
grep -q 'widgetlib_empty' <<<"$got" \
...
got="$(bash -c '
    ...
    : > "'"$TMP"'/widgetlib.sh"
    rb_load "'"$TMP"'" widgetlib WIDGET_VAR "PR_X status=error" var 2>/dev/null
...
got="$(run_case 'widget_fn() { :; }' 'widget_fn() { printf stale; }; readonly -f widget_fn' \
        widgetlib widget_fn "PR_X status=error")"
...
got="$(run_case 'WIDGET_VAR=fresh' 'WIDGET_VAR=stale; readonly WIDGET_VAR' \
        widgetlib WIDGET_VAR "PR_X status=error" var)"
grep -q 'widgetlib_stale_definition' <<<"$got" \
    && pass "…and so is a readonly variable" \
    || die "a readonly variable was loaded over ('$got')"

# ── a library that is not there ────────────────────────────────────────────
got="$(bash -c '. "'"$SELF_DIR"'/loadlib.sh"; rb_load "'"$TMP"'" nosuchlib nosuch_fn "PR_X status=error" 2>&1' || true)"
...
got="$(run_case 'widget_fn() { :; }' '' widgetlib widget_fn "PR_X status=error" funtion)"
...
# THE WHOLE PREFIX IS THE CALLER'S. `pr-watch.sh` reports `state=error` where the
# others report `status=error`, so a loader that supplied the key would either
# impose one spelling on a script whose every other line uses the other, or emit
# `state=error status=error`. It knows the reason; the caller knows how it says
# "this failed".
got="$(run_case '' '' widgetlib widget_fn "PR_SOMETHING state=error")"
{ grep -q 'PR_SOMETHING state=error' <<<"$got" && grep -q 'reason=widgetlib_' <<<"$got"; } \
    && pass "a refusal is addressed in the caller's own words, plus the library" \
    || die "a refusal is not addressed ('$got')"
grep -q 'status=error' <<<"$got" \
    && die "the loader imposed its own status key on the caller ('$got')" \
    || pass "…and the loader adds no key of its own"
