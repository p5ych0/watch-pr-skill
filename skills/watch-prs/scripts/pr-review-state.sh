...
unset -f rb_identity 2>/dev/null || {
    echo "PR_REVIEW_STATE status=error reason=identitylib_stale_definition" >&2; exit 2; }
# shellcheck source=identitylib.sh
. "$_RB_SELF_DIR/identitylib.sh" || {
    echo "PR_REVIEW_STATE status=error reason=identitylib_unreadable" >&2; exit 2; }
[ "$(type -t rb_identity 2>/dev/null)" = function ] || {
    echo "PR_REVIEW_STATE status=error reason=identitylib_empty" >&2; exit 2; }
rb_identity || {
