...
case "${TMPDIR:-}" in /*) RB_SETUP_DIR="$TMPDIR/watch-pr-setup.$$.$RANDOM$RANDOM" ;; *) (umask 077 && /usr/bin/env mkdir -p "$HOME/.cache/watch-pr") || { echo "ABORT: setup failed; $HOME/.cache/watch-pr could not be created"; exit 1; }; RB_SETUP_DIR="$HOME/.cache/watch-pr/watch-pr-setup.$$.$RANDOM$RANDOM" ;; esac
/usr/bin/env bash -p "$RB_SCRIPTS"/pr-setup.sh "$RB_SETUP_DIR" \
    || { [ $? -eq 2 ] && (umask 077 && /usr/bin/env mkdir -p "$HOME/.cache/watch-pr") && RB_SETUP_DIR="$HOME/.cache/watch-pr/watch-pr-setup-2.$$.$RANDOM$RANDOM" && /usr/bin/env bash -p "$RB_SCRIPTS"/pr-setup.sh "$RB_SETUP_DIR"; } \
...
so an origin file replaced after setup addresses nothing. An unset or relative `$TMPDIR`
is not a parent; `~/.cache/watch-pr` is, so what setup leaves stays out of `$HOME` itself.
It is created under `umask 077`, since `pr-origin.sh` refuses a group-writable ancestor
and `mkdir -m` sets only the last component. Status 2 from either helper means the storage refused, and the one
retry — under `~/.cache/watch-pr`, or under a second leaf — is the bound
...
4. **check the round boundary — step 6.** All three precede the push: with automatic review
   ...
   the round may close at all;
5. **run `gate`**, and only then reply to each thread with what changed, react to it,
   resolve it, and read the `isResolved` the resolve returns. A resolve cannot be taken
   back, so it follows the pushed, green head;
6. **run `post`** — it re-proves that head, posts the summary and re-requests `$WHO`. The
