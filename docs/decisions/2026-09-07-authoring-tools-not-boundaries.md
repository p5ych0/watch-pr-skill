...
- **The cold reviewer sees what the session sees.** It is a subagent of the same session,
  ...
  session itself whether the subagent reads them or not.
...
Fifteen review rounds on #314 narrowed the cold reviewer's brief case by case — rename
...
- `test-hooks.sh` stays in the suite and runs on both CI jobs.
