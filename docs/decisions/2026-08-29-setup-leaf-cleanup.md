...
| a refusal from `pr-origin.sh read` itself | the directory is left, and whether its `o` transport is still inside depends on where that helper stopped — it creates its own transport and gives it back where it can (#157), which since #266 means before its write; a write that opens and then fails leaves the transport and its leaf, for as long as that leaf is there. That is the one removal this contract does not cover and does not own |
