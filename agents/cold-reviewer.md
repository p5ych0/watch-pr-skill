...
except that a name ending `.example` is a committed template the reviewers read too, and
is marked only by a directory above it. A marked path is reported as changed and left
unopened, contents and all. Read the changed
