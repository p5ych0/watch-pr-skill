...
| `scripts/pr-setup.sh` | Session start: reserves a directory, reads the origin through `pr-origin.sh read`, creates the four empty working files, reports `mode=attended` or `mode=unattended`. Removes nothing (`docs/decisions/2026-08-29-setup-leaf-cleanup.md`). 0 ready, 1 stopped, 2 the storage refused. |
...
| `scripts/pr-review-state.sh` | Whether a reviewer's review of a head can carry a merge: every comment counts, replies included; a review of only replies is `source=replies-only`, neither answer. `state`, `verdict`, `head`, `review-id`, `clean-at`, `escape-snapshot`. |
...
| `scripts/recordlib.sh` | What a well-formed GitHub record is, which lines are control records, what text requests a review, what the replies-only escape means. |
| `scripts/writelib.sh` | How a value crosses in a caller-named file: exclusive create, write, exact rename, one non-blocking read-back that hands the value back. |
