...
| `skills/watch-prs/scripts/pr-ci-state.sh` | Whether the pushed head's checks are green, still running, failing, or absent. `--head` BINDS THE ANSWER TO A COMMIT, and every caller passes it: `gh pr checks` takes a PR number and has no commit selector, so without it the answer is about whatever the API currently calls the head. The helper confirms the head before AND after the checks read and reports `stale` if it moved — one confirmation leaves a window the request itself spans. The merge gate was the one caller that did not pass it, which let an A → B → A force-push merge A on B's checks while `--match-head-commit A` succeeded; a `stale` answer is not a failed probe and the caller re-runs the gate. #212. |
