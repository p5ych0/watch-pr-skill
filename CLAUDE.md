...
| `skills/watch-prs/scripts/pr-copilot-phase.sh` | The Codex→Copilot transition, in two stages with the operator's decision between them: `record` proves Codex clean on an exact head, proves that head's checks and writes the signoff onto the PR, then stops and asks; `open` runs only on the answer: a recorded signoff is history, not a current verdict, so it re-reads Codex's verdict against that sha as well as the head — and re-reads the head once more immediately before the mutations, since the probes are a window a push can land in. 0 recorded/opened, 1 stopped, 3 paused. Was 176 lines in `SKILL.md` — see #26. |
