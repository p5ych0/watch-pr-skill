...
| `skills/watch-prs/scripts/pr-review-state.sh` | Whether a named reviewer's review of the current head can carry a merge. EVERY comment counts as a finding, replies included: a verdict followed by explanation and a verdict followed by a retraction are the same text to anything that reads it, so no exemption is safe. A review whose comments are ALL replies reports `source=replies-only` — nothing to fix and not a signoff — and the driver stops for the operator. `replies-at` reports when the NEWEST of those comments landed, which is a different moment from when the review did: a reply added afterwards does not move the review's `submitted_at`, so an operator signoff ordered against `review-at` alone vouched over a retracting reply nobody read. 0 the time, 1 nothing to order against — no review, no comments, or a verdict that arrived as an issue comment and therefore carries none — 2 unreadable. The fetch's status is taken on its own line even though `pages_or_error` refuses an empty read: a page printed before a later one fails still PARSES, and the answer would be a maximum over half the comments. The snapshot is read again afterwards too — the comments belong to a review this call has stopped looking at, and one dismissed in between describes the old replies while presenting itself as the current answer. #130, for #129. `escape-snapshot` is the replies-only escape's whole question in one answer: which review it is, when it landed and when its newest reply did, derived from ONE pair of review reads with the comments counted, timed and classified between them. It exists because asking those as four probes and binding them by re-reading each left a window every time — #132 spent five rounds one layer in — and a sequential guard cannot close a gap between sequential calls. 0 the three values, 1 not that shape, 2 unreadable. #133. |
...
| `set -uo pipefail` | `pr-review-state.sh`, `pr-merge-range.sh`, `pr-round-count.sh`, `pr-findings.sh`, `pr-watch.sh`, `pr-ci-state.sh`, `pr-ci-gate.sh`, `pr-merge-gate.sh`, `pr-signoff.sh`, `pr-close-round.sh`, `pr-copilot-phase.sh`, `pr-phase-state.sh`, `pr-origin.sh`, `pr-selfcheck.sh` | **`-e` is forbidden here.** Subcommands use exit codes as control flow and several `gh` probes "fail" as normal operation; `pr-selfcheck.sh` is in this row because a `grep` that matches nothing exits 1 as its normal answer. |
