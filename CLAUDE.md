...
| `skills/watch-prs/scripts/pr-ci-state.sh` | Whether the pushed head's checks are green, still running, failing, or absent. `--head` BINDS THE ALL-CHECKS ANSWER AND BRACKETS THE `--required` ONE, and every caller passes it. The all-checks question is read from the commit's own `statusCheckRollup`, addressed by the OID it is asked for, so the answer is about that commit; the confirmations either side then only report whether the head has since moved. ONE ROLLUP RATHER THAN TWO PAGINATED REST READS, which is what ended a class four review rounds were spent inside: `--paginate` requests pages sequentially and is not a snapshot, so a rerun landing between two of them lets a record repeat, or be REPLACED by one carrying a fresh id while `total_count` holds — and that second one is invisible to any rule about the pages, because every page is individually well-formed. Nothing available makes two REST reads atomic, so each guard bought one interleaving and left the next; the server computes the rollup over both sources itself. Measured: a `cli/cli` commit with thirteen check runs and no statuses reports SUCCESS, and a `pandas-dev/pandas` commit with one failed run, six still in progress and a passing legacy status reports FAILURE — which is this file's own precedence, failed over pending. `EXPECTED` is pending, being a required context that has not reported; a null rollup is `none` and a null `object` is an ERROR, since a commit the repository does not have would otherwise arrive as "no checks are configured". `--required` IS BOUND TOO since #214, and what made that possible is that the required set turned out to be READABLE: that issue measured `branches/<b>/protection`, which does need admin and denies with a 404 indistinguishable from "not protected", but the BRANCH OBJECT carries the same answer with the `repo` scope this loop runs under — measured on ten repositories none of which this account administers, three required contexts on `cli/cli`, eleven on `kubernetes/kubernetes`, twenty-three on `microsoft/vscode`, while the dedicated endpoint 404s on the same repository with the same token. BOTH SOURCES, because either can be the whole answer: `home-assistant/core` reports `protection.enabled: false` with `protected: true` and its eight contexts appear only under `rules/branches/<b>`, `cli/cli` is the other way round, so the required set is the UNION and reading one alone reports a requirement as absent. With that set in hand the question becomes "do these contexts pass on THIS commit", which the same rollup answers — so a required context that has NOT reported is `pending` rather than invisible, which is the case the all-checks gate cannot see and the reason #214 was a merge-safety hole rather than a reporting inaccuracy. `protected: false` is an answer and an unreadable protection shape is not: the first reports `none`, the second is an error, because a merge gated on an empty required set is the direction that opens the gate. The BRANCH read is also what proves the name, since `rules/branches/<b>` answers `[]` for a branch that does not exist while `branches/<b>` 404s. A `stale` answer is not a failed probe and the caller re-runs the gate. What is still not bound is the required SET, and it cannot be — protection is a property of the base branch rather than of a commit — nor is the "require branches to be up to date" policy read. #212, #214. |
