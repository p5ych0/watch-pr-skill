...
idiom while you are in it for something else.
...
a defect to delete, never prose to preserve. The `# WHY:` claim-and-rationale mechanism
`SKILL.md` carried was retired under this rule in 2.3.0 — no claims, no rationale
document. The fences are to hold commands and the occasional one-line constraint; the
multi-line comments still inside some of them are the remainder the next change removes.
...
(`perl` is the precedent, and the argument for it is in the changelog). None where an
...
| Path | Role |
| --- | --- |
| `skills/watch-prs/SKILL.md` | The driver contract — how the model requests reviews from the native GitHub reviewers, reads findings, closes rounds, and gates the merge. Until 2.3.0 its fences carried a `# CLAIM` / `# WHY:` pair per invariant with the argument in a shipped rationale document, proved by a bijection in `test-pr-skill-contract.sh`; retired under § Working rules → Comments — the arguments are in the changelog and the bijection machinery is gone; the fences are to hold commands and one-line constraints, and the multi-line comments still inside some of them are the remainder the next change removes (two rationale-shape checkers were built and removed before it; do not build a third). |
...
| `skills/watch-prs/scripts/pr-watch.sh` | Blocks until a reviewer's verdict on the current head is actionable. 0 verdict in hand, 1 timed out, 2 unreadable, **4 the review carried only replies** — nothing to fix and no signoff, so the driver stops for the operator. That one has its own status because every caller branches on status: saying it in the record alone left `pr-close-round.sh` taking the 0 and closing the round. `--after-review-file` ALWAYS TRAVELS WITH `--require-nonce <digits>` since #264's second half: the file holds `<nonce> <value>`, the nonce is the one the driver generated for THIS request and handed to the writer, and a file carrying any other — the previous round's baseline, a complete id that no shape test could reject, left in place by a refusal the driver's `exit` returned from — is `state=error reason=stale_baseline_nonce` rather than waited past. The file form without the option is refused, an unnonced file is refused rather than read as an id, and the VALUE form refuses a nonce: accepting any of those would keep the fail-open and add a spelling, as accepting both empty and `none` would have. |
...
paragraph and separating them is what #232 is; the driver's part is what `SKILL.md`'s
setup fence does, and the changelog is where its argument is kept.
...
- **An assignment's status cannot be taken.** Measured on bash 5: a failed
  ...
  case.
- **A comment that contradicts the code beside it is followed as an instruction.**
  That is the reason a comment is kept only while it is TRUE and states a
  non-obvious constraint — a stale one misleads the next reader into "fixing" the
  code to match it. Delete a comment whose argument has moved on; do not extend it.
...
and an accepted limit in `docs/decisions/`. A user-visible change with no
`README.md` update is incomplete, not merely undocumented.
...
decoration here: this file records that *a comment that contradicts the code
beside it is followed as an instruction*, so the constraint a comment states is
part of what the helper does. A user who diffs their installed plugin sees a
changed file, and every changed installed file must have a version that names it. "Observable" is also not decidable at review time,
