...
| `skills/watch-prs/scripts/pr-merge-gate.sh` | Every merge gate, evaluated immediately before merging, with the merge pinned to one head by `--match-head-commit`. WHAT EACH GATE IS ABOUT DIFFERS, and the file's own synopsis is the list: the reviewer verdicts and the reviewed range are commit-addressed — Codex against the head IT signed, where the Copilot phase moved past it, with the range gate licensing the delta — the two check gates are BRACKETED by the merge head rather than bound to it, since `gh pr checks` is addressed by pull request, and the unresolved-thread and round-boundary gates are PR-level and never took an OID. Saying "every gate is evaluated against the merged head" is the paraphrase #212 spent twelve rounds deleting from eight places; do not write it back. Takes a reviewers mode: `both`, or `codex-only` which requires the head to BE the reviewed commit. 0 merged, 1 blocked, 3 paused for the operator, 4 queued — a merge queue takes the request without landing it, and `gh` calls that success. Was 291 lines in `SKILL.md` — see #26. |
