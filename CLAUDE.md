...
| Path | Role |
| --- | --- |
| `skills/watch-prs/SKILL.md` | The driver contract — how the model requests reviews from the native GitHub reviewers, reads findings, closes rounds, and gates the merge. Until 2.3.0 its fences carried a `# CLAIM` / `# WHY:` pair per invariant with the argument in a shipped rationale document, proved by a bijection in `test-pr-skill-contract.sh`; retired under § Working rules → Comments — the fences hold commands and one-line constraints, the arguments are in the changelog, and the bijection machinery is gone (two rationale-shape checkers were built and removed before it; do not build a third). |
