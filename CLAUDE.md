...
| `skills/watch-prs/scripts/pr-ci-state.sh` | Whether the pushed head's checks are green, still running, failing, or absent. `--head` BINDS THE ALL-CHECKS ANSWER AND BRACKETS THE `--required` ONE, and every caller passes it. The all-checks question is read from the commit's own `statusCheckRollup`, addressed by the OID it is asked for, so the answer is about that commit; the confirmations either side then only report whether the head has since moved. ONE ROLLUP RATHER THAN TWO PAGINATED REST READS, which is what ended a class four review rounds were spent inside: `--paginate` requests pages sequentially and is not a snapshot, so a rerun landing between two of them lets a record repeat, or be REPLACED by one carrying a fresh id while `total_count` holds — and that second one is invisible to any rule about the pages, because every page is individually well-formed. Nothing available makes two REST reads atomic, so each guard bought one interleaving and left the next; the server computes the rollup over both sources itself. Measured: a `cli/cli` commit with thirteen check runs and no statuses reports SUCCESS, and a `pandas-dev/pandas` commit with one failed run, six still in progress and a passing legacy status reports FAILURE — which is this file's own precedence, failed over pending. `EXPECTED` is pending, being a required context that has not reported; a null rollup is `none` and a null `object` is an ERROR, since a commit the repository does not have would otherwise arrive as "no checks are configured". `--required` cannot use those endpoints — they do not say which contexts branch protection REQUIRES, and that read is not available: classic protection needs admin and denies with a 404 indistinguishable from "not protected", while the ruleset endpoints are readable without admin and do not see classic protection at all (measured, #214) — so it keeps `gh pr checks --required`, where the confirmations are a bracket rather than a binding. `gh pr checks` takes a PR number and has no commit selector, so without it the answer is about whatever the API currently calls the head, and the merge gate was the one caller not passing it. The unsafe case is always A → B → A, since one move away is refused by `--match-head-commit`; what the bracket changes is where that pair has to fit — BOTH moves between the two confirmations, rather than straddling everything before the merge. A `stale` answer is not a failed probe and the caller re-runs the gate. **THE BRACKET IS NOT A BINDING**, and the comment beside it says so: the response carries no OID, so an A → B → A completing between the two confirmations is invisible to both. Making it a binding needs a commit-addressed query and the required-contexts read to go with it, which is #214. #212. |
...
| `skills/watch-prs/scripts/pr-merge-gate.sh` | Every merge gate, evaluated immediately before merging, with the merge pinned to one head by `--match-head-commit`. WHAT EACH GATE IS ABOUT DIFFERS, and the file's own synopsis is the list: the reviewer verdicts and the reviewed range are commit-addressed — Codex against the head IT signed, where the Copilot phase moved past it, with the range gate licensing the delta — the all-checks gate is commit-addressed too since #214, the REQUIRED-checks gate is only BRACKETED by the merge head because `gh pr checks --required` is addressed by pull request and the required-contexts read that would replace it is not available, and the unresolved-thread and round-boundary gates are PR-level and never took an OID. THE BRACKET ON (4) IS A MERGE-SAFETY HOLE, and the argument that (3b) covered it was refuted in review: (3b) reads the checks that EXIST on the merge target, which is not the set a branch REQUIRES — a required context that has not reported has neither a check run nor a status, so (3b) can be green while the requirement is unmet. #214, and not closable from the client: refusing whenever the required set cannot be read would refuse on every repository this loop does not administer, so `REVIEW_MERGE_STRICT=1` on non-bypassable protection is what closes it. Saying "every gate is evaluated against the merged head" is the paraphrase #212 spent twelve rounds deleting from eight places; do not write it back. Takes a reviewers mode: `both`, or `codex-only` which requires the head to BE the reviewed commit. 0 merged, 1 blocked, 3 paused for the operator, 4 queued — a merge queue takes the request without landing it, and `gh` calls that success. Was 291 lines in `SKILL.md` — see #26. |
...
- **Portable, and proven by running rather than by reading.** The `macos-shell` CI
  ...
  **`SKILL.md`'s bash is covered ONLY WHERE THE CONTRACT LIFTS IT, and that boundary
  ...
  the glue that has to run in the operator's shell: the setup block exports
  `REVIEW_BUS_REMOTE` and a child cannot export into its parent, and the read-backs
  exist to catch a readonly name or a nameref defeating the driver's own
  assignment — a helper process cannot observe that. The four programs the issue
  named are all scripts with tests now.
