...
- **Portable, and proven by running rather than by reading.** The `macos-shell` CI
  ...
  **`SKILL.md`'s bash is covered by neither CI job, and that is settled rather than
  ...
  `exit`, a neutralised `echo`, an interactive shell and a forged helper. The
  `[[ … =~ … ]]` parse defect that prompted #26 would be caught there today. One
  narrow lift by anchored `grep`, with no grammar, covers the merge-gate condition
