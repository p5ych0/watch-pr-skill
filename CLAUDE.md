...
| `skills/watch-prs/scripts/writelib.sh` | How a value crosses to the caller in a file the CALLER named — WRITE, THEN RENAME, sourced by `pr-close-round.sh` and `pr-copilot-phase.sh`. It exists because seven sites across those two spelled the handoff `printf … > "$THE_PATH"`, with two more emptying the same files with a bare `>`, and `>` FOLLOWS A SYMLINK: a same-UID process that replaced one of those paths had the symlink's TARGET truncated — a file of the operator's, outside the session's working directory, on every invocation that got that far rather than only on a refusal. `rename(2)` replaces the NAME, so the link goes and what it pointed at does not, which is measured both ways in `test-writelib.sh`. The temporary is created EXCLUSIVELY under `set -C` in the same open that writes it: creating it and then opening it again by name would be a check-then-open of its own, with the second open free to follow a symlink or block on a FIFO — the defect reproduced inside the fix. That exclusive open settles the FIFO case — O_CREAT\|O_EXCL fails instead of waiting — but NOT blocking in general: pathname resolution, the write and the `mv` can stall on an unresponsive filesystem, so the three call sites that were bounded still are, with `run_limited` moved around the library call. The TARGET'S TYPE is refused before anything is created, and that is NOT the shape #245 convicted: nothing here ever opens the target, so the rename's safety does not rest on the answer. What it refuses is a caller naming something that is not a handoff file — a directory, which `mv` moves the source INSIDE while reporting success; a device or socket, which `mv -f` renames over, so `/dev/null` named here became a regular file; a FIFO, refused rather than replaced so the promise `README.md` already makes still holds. A symlink to a REGULAR file passes and must, since that is the whole of #263. The one interleaving the type test cannot close — a racer making the target a directory after it — is caught by a POSTCONDITION, and the temporary's name carries two `$RANDOM` draws so that residue is litter rather than loss: a guessable name could be pre-placed in the racer's directory as a file worth keeping and overwritten by `mv -f`. Two entry points, because an emptying is not a value: `gate` removes a claim and wants zero bytes, and a lone newline would be a different thing that happens to be refused the same way today. NOTHING IS REMOVED, including the temporary a failed write leaves — `docs/decisions/2026-08-29-setup-leaf-cleanup.md` convicts the class, and `test-writelib.sh` asserts the file contains no removal at all. #263. |
