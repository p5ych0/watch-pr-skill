...
| `skills/watch-prs/scripts/pr-origin.sh` | The session's repository, read where the driving shell's names cannot reach — `read` writes origin's URL into a directory the caller names and this helper creates, `pin` writes `REVIEW_BUS_REMOTE` as a child process sees it, and refuses it if it is not this checkout's origin. It is the one helper with a section of its own: **§ `pr-origin.sh`, in full**, below this table. #84, #232. |
