...
- **The cold reviewer ships with the plugin.** The subagent that reads a branch against its
  ...
  stopped where either was missing. It now asks the base which of the root's policy files it
  has, `.github/copilot-instructions.md`, `AGENTS.override.md` (read in place of `AGENTS.md`),
  `AGENTS.md` and `CLAUDE.md`, reads those and says which it found; a policy file below the
  root governs only its own subtree and is not read. Its secrets rule now also covers
  anything under a `.env`, `.env.*` or `.ssh` directory; its other safety rules are
  unchanged. This checkout reads the brief through a link in `.claude/agents/`, so a
