...
- **Each probe the watch makes is bounded on its own, and a stalled one is retried.** A watch
  ...
  closed: each probe is now bounded by the smaller of the remaining deadline and
  `PR_WATCH_PROBE_TIMEOUT` (default 60 seconds, the CI gate's precedent), a probe that hits its
  own bound short of the deadline is killed and retried on a fresh process, and the retry is
  printed as `state=probe_stalled probe=<which> limit_s=<bound>`, so a stall and a lag read
  differently next time. A probe that hits the deadline is still the timeout, and retried
  stalls still count against it. `test-pr-watch.sh` stages a state probe that stalls once and
  proves the watch reaches the verdict at the cost of the bound, and one that stalls on every
  call and proves it still reaches the timeout. #281.
...
- **Four more helpers carry a comment only where the code looks wrong without one.**
  ...
  a working-file path that resolves to an existing regular file, why a signoff stands only if no revocation is newer than the
...
- **The six smallest helpers carry a comment only where the code looks wrong without one.**
  ...
  code, and each opened with the same essay on privileged startup, five of them with the same
  ...
  non-obvious constraint in a line or two, so 104 lines remain, each beside the code it
...
- **The six shared libraries carry a comment only where the code looks wrong without
  ...
  non-obvious constraint in a line or two, so that is what remains, 88 lines, each beside
...
- **`SKILL.md` is 1,918 characters shorter, and nothing was removed but a repeated
  ...
  above the first fence costs 1,176 back, which is where the net 1,918 comes from.
