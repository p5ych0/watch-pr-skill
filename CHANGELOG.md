...
- **A refusal after the directory existed left it behind.** The helper's contract
  ...
  refusal, a signal, or the end of the script — with every trap removed before it
  begins, so a signal arriving during it cannot re-enter it. `RB_PHASE` picks its
  shape:
